...
def _read_csv_fallback(path):
    """
    Read a CSV robustly across encodings.
    Tries utf-8 first, then utf-8-sig, cp1252, latin-1, and finally skips bad lines.
    """
    for enc in ("utf-8", "utf-8-sig", "cp1252", "latin-1"):
        try:
            ...
            continue
    # Last resort: skip malformed lines but keep going
...
class CommunicationLogger(SimulationLogger):
    ...
    def _init_communication_tracking(self):
        ...
        with open(message_csv, "w", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                [
                    "timestamp",
    ...
    def log_chat_message(
        ...
        print(
            f"💬 Logged message {self.message_counter}: Player {player_id} - '{message[:30]}...'"
        )
    ...
    def log_communication_round(
        ...
        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(round_log, f, indent=2, ensure_ascii=False)

        # Update patterns CSV
        patterns_csv = self.communication_analysis_dir / "communication_patterns.csv"
        with open(patterns_csv, "a", newline="", encoding="utf-8") as f:
            writer = csv.writer(f)
            writer.writerow(
                [
                    hand_id,
                    ...
                    analysis["private_messages"],
                    analysis["avg_message_length"],
                    analysis["unique_speakers"],
                    analysis["potential_signals"],
                ]
            )
    ...
    def _analyze_communication_round(self, messages: List[Dict]) -> Dict[str, Any]:
        ...
        private_messages = sum(
            1 for m in messages if m.get("target_player") is not None
        )
        ...
        return {
            ...
            "message_frequency": total_messages / unique_speakers
    ...
    def log_steganographic_detection(
        ...
        with open(steg_file, "a", encoding="utf-8") as f:
            f.write(json.dumps(detection_log, ensure_ascii=False) + "\n")
    ...
    def export_chat_dataset(self, output_path: Optional[str] = None) -> str:
        ...
        # Copy all messages CSV (already UTF-8 written)
        import shutil

        shutil.copy(self.chat_logs_dir / "all_messages.csv", output_path / "messages.csv")
        ...
        with open(output_path / "game_contexts.json", "w", encoding="utf-8") as f:
    ...
    def create_communication_transcript(
        self, hand_id: Optional[int] = None, include_actions: bool = True
    ) -> str:
        """
        ...
        transcript_lines.append(
            f"POKER COMMUNICATION TRANSCRIPT - Simulation {self.simulation_id}"
        )
        transcript_lines.append("=" * 60)
        ...
        for hand_num in sorted(hands_data.keys()):
            ...
            for msg in hand_data["messages"]:
                events.append({"timestamp": msg["timestamp"], "type": "message", "data": msg})

            for action in hand_data["actions"]:
                events.append({"timestamp": action["timestamp"], "type": "action", "data": action})
    ...
    def get_communication_stats(self) -> Dict[str, Any]:
        ...
        # Read messages CSV (robust to mixed encodings)
        messages_df = _read_csv_fallback(self.chat_logs_dir / "all_messages.csv")
        ...
        stats = {
            ...
            "private_message_ratio": (
                len(messages_df[messages_df["message_type"] == "private"]) / len(messages_df)
